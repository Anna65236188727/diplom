<!DOCTYPE html>
<html lang="ru">

<head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <link rel="stylesheet" href="style.css" />
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/swiper@11/swiper-bundle.min.css" />
    <title>Корзина</title>
</head>
<?php
include 'header.php';
?>
<div class="basket">
    <div class="container">
        <ul class="breadcrumb_black">
            <li><a class="ul18_gray" href="index.html">Главная</a></li>
            <li><a class="ul18_gray" href="catalog.php">Каталог</a></li>
            <li class="ul18_gray">Корзина</li>
        </ul>
        <div class="ur30_black name_basket">Корзина</div>
        <?php
        session_start();

        if (!isset($_SESSION['cart']) || empty($_SESSION['cart'])) {
            echo "Корзина пуста.";
            exit;
        }

        $servername = "localhost";
        $username = "root";
        $password = "";
        $dbname = "my_shop";

        $conn = new mysqli($servername, $username, $password, $dbname);

        if ($conn->connect_error) {
            die("Connection failed: " . $conn->connect_error);
        }
        ?>
        <div class="block_baskets">
            <div class="basket_block">
                <div class="ur20_black">Ваш заказ:</div>
                <hr>
                <div class="list_basket">
                    <?php
                    foreach ($_SESSION['cart'] as $productId => $quantity) {
                        $sql = "SELECT id, name_product, price, image_1, image_2, image_3, image_4, image_5, materials, size, weight, description, quantity FROM products WHERE id=?";
                        $stmt = $conn->prepare($sql);
                        $stmt->bind_param('i', $productId);
                        $stmt->execute();
                        $result = $stmt->get_result();
                        if ($result->num_rows > 0) {
                            $row = $result->fetch_assoc();
                            $displayQuantity = htmlspecialchars($row['quantity']);

                            ?>

                            <div class="product_basket" data-product-id="<?php echo $row['id']; ?>">
                                <div class="img_name">
                                    <img class="width140" src="uploads/<?php echo $row['image_1']; ?>" alt="">
                                    <div class="name_composition">
                                        <div class="ur20_black"><?php echo htmlspecialchars($row['name_product']); ?></div>
                                        <div class="ur18_gray"><?php echo htmlspecialchars($row['materials']); ?></div>
                                    </div>
                                </div>
                                <div class="quantity">
                                    <div class="quantity_block" data-product-id="<?php echo $row['id']; ?>">
                                        <svg class="decrease" id="decrease-<?php echo $row['id']; ?>" width="29" height="29"
                                            viewBox="0 0 29 29" fill="none" xmlns="http://www.w3.org/2000/svg"
                                            xmlns:xlink="http://www.w3.org/1999/xlink">
                                            <rect width="29" height="29" fill="url(#pattern0_814_88)" />
                                            <defs>
                                                <pattern id="pattern0_814_88" patternContentUnits="objectBoundingBox" width="1"
                                                    height="1">
                                                    <use xlink:href="#image0_814_88" transform="scale(0.01)" />
                                                </pattern>
                                                <image id="image0_814_88" width="100" height="100"
                                                    xlink:href="data:image/png;base64," />
                                            </defs>
                                        </svg>
                                        <div class="current_quantity ur26_black"
                                            data-initial-quantity="<?php echo htmlspecialchars($row['quantity']); ?>">1</div>
                                        <svg class="increase" id="increase-<?php echo $row['id']; ?>" width="29" height="29"
                                            viewBox="0 0 29 29" fill="none" xmlns="http://www.w3.org/2000/svg"
                                            xmlns:xlink="http://www.w3.org/1999/xlink">
                                            <rect width="29" height="29" fill="url(#pattern0_814_86)" />
                                            <defs>
                                                <pattern id="pattern0_814_86" patternContentUnits="objectBoundingBox" width="1"
                                                    height="1">
                                                    <use xlink:href="#image0_814_86" transform="scale(0.01)" />
                                                </pattern>
                                                <image id="image0_814_86" width="100" height="100"
                                                    xlink:href="data:image/png;base64," />
                                            </defs>
                                        </svg>
                                    </div>
                                    <div class="ur16_black">В наличии: <?php echo htmlspecialchars($displayQuantity); ?> шт.
                                    </div>
                                </div>
                                <form action="functions.php" method="post">
                                    <input type="hidden" name="productId" value="<?php echo $productId; ?>">
                                    <button class="svg_delete" type="submit" class="delete-button"
                                        title="Удалить товар из корзины">
                                        <svg id="delete-icon" width="29" height="29" viewBox="0 0 29 29" fill="none"
                                            xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                            <rect width="29" height="29" fill="url(#pattern0_814_90)" />
                                            <defs>
                                                <pattern id="pattern0_814_90" patternContentUnits="objectBoundingBox" width="1"
                                                    height="1">
                                                    <use xlink:href="#image0_814_90" transform="scale(0.01)" />
                                                </pattern>
                                                <image id="image0_814_90" width="100" height="100"
                                                    xlink:href="data:image/png;base64," />
                                            </defs>
                                        </svg>
                                    </button>
                                </form>
                            </div>
                            <?php
                        }
                    }

                    $conn->close();
                    ?>
                </div>
            </div>
            <div class="basket_block2">
                <div class="product_basket_block">
                    <div class="ur18_gray">Товары, 1 шт.</div>
                    <div class="ur18_gray"><?php echo $row['price']; ?> ₽</div>
                </div>
                <div class="product_basket_block">
                    <div class="ub36_black">Итого</div>
                    <div class="ub36_black">850 ₽ </div>
                </div>
                <a href="making_order.php"><button class="button ur26_white btn">Перейти к оформлению</button></a>
            </div>
        </div>
    </div>
</div>
<?php
include 'footer.php';
?>
<script>
    // УВЕЛИЧЕНИЕ И УМЕНЬШЕНИЕ ТОВАРА

    // document.addEventListener("DOMContentLoaded", function () {
    //     var quantityBlocks = document.querySelectorAll('.quantity_block');
    //     console.log('Количество блоков:', quantityBlocks.length);
    //     quantityBlocks.forEach(function (block) {
    //         // Измененный способ выбора элементов уменьшения и увеличения
    //         var decreaseButton = block.querySelector('.decrease');
    //         console.log(decreaseButton);
    //         var increaseButton = block.querySelector('.increase');

    //         // Проверяем, что элементы найдены
    //         if (!decreaseButton || !increaseButton) {
    //             console.error('Не удалось найти элементы уменьшения или увеличения');
    //             return; // Прерываем выполнение для данного блока, если элементы не найдены
    //         }

    //         var currentQuantity = block.querySelector('.current_quantity');

    //         // Обработчик увеличения количества
    //         increaseButton.addEventListener('click', function () {
    //             currentQuantity.textContent = parseInt(currentQuantity.textContent) + 1;
    //             updateQuantityInDatabase(block.dataset.productId, parseInt(currentQuantity.textContent));
    //         });

    //         // Обработчик уменьшения количества
    //         decreaseButton.addEventListener('click', function () {
    //             if (parseInt(currentQuantity.textContent) > 1) {
    //                 currentQuantity.textContent = parseInt(currentQuantity.textContent) - 1;
    //                 updateQuantityInDatabase(block.dataset.productId, parseInt(currentQuantity.textContent));
    //             }
    //         });
    //     });
    // });

    // // Функция для обновления количества в базе данных
    // function updateQuantityInDatabase(productId, newQuantity) {
    //     var xhr = new XMLHttpRequest();
    //     xhr.open('POST', 'functions.php', true); // Убедитесь, что путь правильный
    //     xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
    //     xhr.onreadystatechange = function () {
    //         if (xhr.readyState == 4 && xhr.status == 200) {
    //             alert(xhr.responseText); // Обработка ответа сервера
    //         }
    //     };
    //     xhr.send('product_id=' + productId + '&new_quantity=' + newQuantity);
    // }
    <script>